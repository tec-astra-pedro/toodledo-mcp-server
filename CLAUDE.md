...
- `src/client.test.ts` covers `ToodledoClient` (HTTP/auth behavior, mocked via `msw`). `src/index.test.ts` and `src/index.main.test.ts` cover `src/index.ts`'s MCP tool handlers (`ListToolsRequestSchema`/`CallToolRequestSchema`) and `main()`, respectively. Combined coverage is ~97% statements/branches/lines — see `npm run test:coverage`.
